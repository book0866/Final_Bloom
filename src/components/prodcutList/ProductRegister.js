...
function ProductRegister() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${host}/product/register`, formData, {
        headers: {
    ...
    <main className="product-register-container">
      <div className="pd-reg-box">
        ...
        <form onSubmit={handleSubmit} className="pd-reg-form">
          <div className="pd-reg-content">
            ...
            <div className="pd-reg-group">
              ...
              <select name="depth1" value={form.depth1} onChange={handleChange}>
                ...
                {Object.keys(depthOptions).map((depth1) => (
                  <option key={depth1} value={depth1}>
                    {depth1}
                  </option>
                ))}
              </select>
            ...
            <div className="pd-reg-group">
              ...
              <select
                ...
                value={form.depth2}
                onChange={handleChange}
                disabled={!form.depth1}
              >
                <option value="">선택하세요</option>
                {depthOptions[form.depth1]?.map((depth2) => (
                  <option key={depth2} value={depth2}>
                    {depth2}
                  </option>
                ))}
              </select>
            ...
            <div className="pd-reg-group comment-group">
              <label htmlFor="comment" className="comment-area">
                상품 설명
              </label>
              <textarea
                name="comment"
          ...
          <button type="submit" className="pd-reg-btn">
            상품 등록
          </button>
        </form>
      </div>
